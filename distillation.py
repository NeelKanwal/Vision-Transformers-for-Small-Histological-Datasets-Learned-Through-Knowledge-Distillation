...
# This file provides training code for transformer under distillation settings mentioned in the paper.
# provide model weights when using teacher with domain knowledge
...
from utils import get_class_distribution, make_cm, dummy_data, make_pretty_cm, convert_batch_list, custom_classifier
from utils import train_distill_transformer, val_distill_transformer, epoch_test_dist_transformer, sendmail, EarlyStopping_v2
...
from utils import SimpleDistiller
...
model_weights = "/home/neel/transform/mobilenet_damaged.dat"
# model_weights = "/home/neel/transform/mobilenet_airbubble.dat" # best_airbubble
# model_weights = "/home/neel/transform/vgg16_blood.dat"

if __name__ == "__main__":
    
    ## Experiment selection
    artifact = "airbubble" #["airbubble", "blur", "fold"]
    architectures = ["SimpleDistillation"]  #  "DistillationTransformer"
    ...
    use_domain_knowledge = False # use the model weights 

    iterations = 1 # Repeat same experiment multiple times to get mean and stf over performance metrics
    ...
    # Distillation parameters to run same experiment using different values. 
    temperatures = [2, 5, 10, 20, 40] #[2, 3, 4, 5, 10]
    alphas = [0.3, 0.5, 0.7] #[0.3, 0.4, 0.5, 0.6]
            ...
            for sch in lr_scheduler:
                for lr in learning_rate:
                    for i in range(iterations):
                        ...
                        if architecture == "DistillationTransformer":
                            # Alternative way, not used in paper. 
                            print(f"Initializing Distillation Transformer...............")

                            student = DistillableViT(image_size=224, patch_size=14, num_classes=num_classes, dim=512,
                                                     depth=6, heads=16, mlp_dim=2048, dropout=dropout, emb_dropout=dropout)

                            teacher = models.mobilenet_v3_large(pretrained=pretrained)
                            teacher.classifier = custom_classifier(960, num_classes, dropout=dropout)
                            print("Teacher is MobileNet")

                            # Use this to make VGG16 teacher
                            # teacher = models.vgg16(pretrained=pretrained)  
                            # teacher.classifier[6] = custom_classifier(4096, num_classes, dropout=dropout)
                            # print("Teacher is VGG16")

                            if use_domain_knowledge:
                                print("Loading weights on ", device)
                                teacher.load_state_dict(torch.load(model_weights, map_location='cpu')['model'])

                            # Distillation structure    
                            model = DistillWrapper(student=student, teacher=teacher,
                                                       temperature=T,           # temperature of distillation, only used in soft distillation
                                                       alpha=alpha,               # trade between main loss and distillation loss, alpha multiplied to student loss
                                                       hard=False)          # hard distillation ignores temperature
                            # model = DistillationTrainer(teacher=teacher, student=student)
                             # criterion = DistilledLoss(alpha=0.5, temperature=3.0)

                            pytorch_total_params = sum(p.numel() for p in student.parameters())
                            print("Total student model parameters: ", pytorch_total_params)

                            pytorch_total_params = sum(p.numel() for p in teacher.parameters())
                            print("Total teacher model parameters: ", pytorch_total_params)

                        elif architecture == "SimpleDistillation":
                            # Distillation used in the paper

                            print(f"Initializing ViT_tiny Transformer...............")

                            student = timm.create_model('vit_tiny_patch16_224', pretrained=True, num_classes = num_classes) # 'vit_small_patch16_224'
                                                            # 'vit_base_patch16_224', 'vit_large_patch16_224', 'vit_huge_patch14_224_in21k', 'vit_tiny_patch16_224'
                            pytorch_total_params = sum(p.numel() for p in student.parameters())
                            print("Total student parameters in ViT Transformer: ", pytorch_total_params)

                            teacher = models.mobilenet_v3_large(pretrained=pretrained)
                            teacher.classifier = custom_classifier(960, num_classes, dropout=dropout)
                            print("Teacher is MobileNet")
                            pytorch_total_params = sum(p.numel() for p in teacher.parameters())
                            print("Total teacher parameters : ", pytorch_total_params)

                            if use_domain_knowledge:
                                teacher.load_state_dict(torch.load(model_weights, map_location='cpu')['model'])

                            # Distillation structure
                            model = SimpleDistiller(student=student, teacher=teacher,temperature=T, alpha=alpha)
                            # criterion = DistilledLoss(alpha=0.5, temperature=3.0)
                        ...
                        print("\nEpoch 0")
                        _, _, _, tr_acc, tr_loss = epoch_test_dist_transformer(student, train_loader, model)
                        _, _, _, val_acc, val_loss = epoch_test_dist_transformer(student, val_loader, model)
                        ...
                        for epoch in range(1, n_epochs + 1):
                            tr_acc, tr_loss = train_distill_transformer(student, optimizer, train_loader, epoch, model)
                            val_acc, val_loss, stop = val_distill_transformer(student, early_stopping, date_time, val_loader, epoch, path, model)
                        ...
                        print(f"(run {i}) Best accuracy at T and alphas {T,alpha} for {str(architecture)} is {best_acc:.3f} % .")
                        ...
                        y_pred, y_true, probs, _, _ = epoch_test_dist_transformer(student, val_loader, model)
                        ...
                        plot_calibration_curve(y_true, [probs], clf_names = clf_names, title=f'Calibration Plot for {artifact}', title_fontsize='large')
                        plt.savefig(f"{path}/Calibration Plot (Val) for {artifact}.png")

                        plt.clf()
                        plt.figure(8)
                        group_names = ["TN", "FP", "FN", "TP"]
                        make_pretty_cm(cm, categories=classes_list, figsize=(12, 12), sum_stats=True, group_names=group_names, cmap="tab20b", title=f"{architecture} Prediction for {str(artifact)}")
                        ...
                        if test:
                            ...
                            y_pred, y_true, probs, _, _ = epoch_test_dist_transformer(student, test_loader, model)
                            y_pred = convert_batch_list(y_pred)
                            y_true = convert_batch_list(y_true)
                            probs = convert_batch_list(probs)
...
sendmail("Program Completed for Distillation")